...
function SalesChart({ bookings, numDays }) {
  ...
  const allDates=eachDayOfInterval({start:subDays(new Date(), Number(numDays)-1), end:new Date()})
...
<ResponsiveContainer height={300} width={700}>
      <AreaChart data={data}> 
          ...
          <Area dataKey="totalSales" type='monotone' stroke={colors.totalSales.stroke} fill={colors.totalSales.fill} strokeWidth={2} name="TotalSales" unit="$" />
                  <Area dataKey="extrasSales" type='monotone' stroke={colors.extrasSales.stroke} fill={colors.extrasSales.fill} strokeWidth={2} name="ExtrasSales" unit="$" />
        </AreaChart>
        </ResponsiveContainer>
   </StyledSalesChart>
  )
